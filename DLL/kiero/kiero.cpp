...
std::unordered_map <size_t, twoFunctions> gameFunctions{};
...
void kiero::shutdown()
{
	if (g_renderType != RenderType::None)
	{
		...
		for (auto hook : gameFunctions) {
			DetourDetach(reinterpret_cast<PVOID*>(hook.second.original), hook.second.detour);
		}
...
kiero::Status::Enum kiero::bind(uint16_t _index, void** _original, void* _function)
{
	...
	if (g_renderType != RenderType::None)
	{
		...
		*_original = target;
		if (DetourAttach(reinterpret_cast<PVOID*>(_original), _function) != NO_ERROR)
		{
			DetourTransactionCommit();
			return Status::UnknownError;
		}
		gameFunctions[_index].original = (PVOID*)_original;
		gameFunctions[_index].detour = _function;
		DetourTransactionCommit();
#endif
		return Status::Success;
	}
...
void kiero::unbind(uint16_t _index)
{
	if (g_renderType != RenderType::None)
	{
		...
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
		DetourDetach(reinterpret_cast<PVOID*>(gameFunctions[_index].original), gameFunctions[_index].detour);
		gameFunctions.erase(_index);
		DetourTransactionCommit();
#endif
	}
}
